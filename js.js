...
function init() {
    ...
    // 4. LOAD MARQUEE PARTNER (BAGIAN YANG DIEDIT)
    db.ref('partners').on('value', snapshot => {
        const data = snapshot.val();
        const container = document.getElementById('marquee-po');
        
        if (data && container) {
            let originalList = Object.values(data).map(p => p.nama);
            
            if (originalList.length > 0) {
                // TRIK: Duplikasi list 2x lipat (Copy-Paste array)
                // Ini biar animasi CSS translateX(-50%) bisa nampilin SEMUA item sebelum reset.
