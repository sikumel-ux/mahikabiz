...
function updateClock() {
    const now = new Date();
    document.getElementById('header-date').innerText = now.toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long' });
...
document.getElementById('search-input').addEventListener('input', (e) => {
    ...
    if (val.length > 0) {
        ...
        const filtered = db.jadwal.filter(b => 
            b.Tujuan.toLowerCase().includes(val) || b["Nama PO"].toLowerCase().includes(val)
        );
        ...
        if (filtered.length > 0) {
            ...
            grid.innerHTML = `<p style="grid-column:1/-1; text-align:center; padding:40px; color:var(--muted);">Jadwal tidak ditemukan.</p>`;
        }
    } else {
...
function showProfile(namaPO) {
    ...
    content.innerHTML = `
        ...
        <a href="https://wa.me/6281234567890?text=Halo Mahika, pesan tiket ${po["Nama PO"]}" style="display:block; text-align:center; background:var(--p-dark); color:white; text-decoration:none; padding:15px; border-radius:16px; margin-top:20px; font-weight:700;">Pesan via WA</a>
    `;
...
updateClock(); fetchWeather(); fetchData();
